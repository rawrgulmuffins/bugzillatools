...
class Bug(object):
    ...
    def update(self, **kwargs):
        ...
        A wrapper for the RPC ``bug.update`` method that performs some
        sanity checks and flushes cached data as necessary.

        If ``assigned_to`` is specified, user matching will be performed
        on its value.  Furthermore, if ``assigned_to`` is specified and
        ``status`` is not set and the ``assign_status`` config is set
        and matches the current status, ``status`` will be updated.

        ``dupe_of``
          If set, ``status`` and ``resolution`` will be ignored.
        ``status``
          Set the status of the bug.  If changing from an open to a
          closed status, a resolution must also be supplied.
        ``resolution``
          Set the resolution of the bug.  Raise ``TypeError`` unless
          the bug is closed or is being closed.
        """
        fields = frozenset([
        ...
        if 'assigned_to' in kwargs:
            ...
            kwargs['assigned_to'] = \
                self.bz.match_one_user(kwargs['assigned_to'])['name']

            if not kwargs.get('status') and 'assign_status' in self.bz.config:
                # update status
                try:
        ...
        # status fields
        if 'dupe_of' in kwargs:
            # ignore status and resolution
            if 'status' in kwargs:
                del kwargs['status']
        ...
        if 'status' in kwargs:
            # see whether new status is open (and check validity while
            # we're at it)
            try:
                is_open = [
                    v['is_open']
                    for v in self.bz.get_field_values('bug_status')
                    if v['name'] == kwargs['status']
                ][0]
            except IndexError:
                raise ValueError('not a valid status: {}'.format(status))
        else:
            is_open = self.data['is_open']

        if self.data['is_open'] and not is_open and 'resolution' not in kwargs:
            raise TypeError('resolution must be set when closing bug')
        if 'resolution' in kwargs and is_open:
            raise TypeError('resolution cannot be set on an open bug')

        # fix up comment field
